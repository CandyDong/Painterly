...
Decoder = FCN()
Decoder.load_state_dict(torch.load('./model/renderer_oil.pkl'))
...
#         self.load_weights('./model')
    ...
    def load_weights(self, path):
        ...
        self.actor.load_state_dict(torch.load('{}/actor_oil.pkl'.format(path)))
        self.critic.load_state_dict(torch.load('{}/critic_oil.pkl'.format(path)))
        load_gan(path)
        
    def save_model(self, path):
        self.actor.cpu()
        self.critic.cpu()
        torch.save(self.actor.state_dict(),'{}/actor_oil.pkl'.format(path))
        torch.save(self.critic.state_dict(),'{}/critic_oil.pkl'.format(path))
        save_gan(path)
        self.choose_device()
